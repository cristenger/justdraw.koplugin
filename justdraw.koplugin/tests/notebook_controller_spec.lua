return function(ctx)
    ...
    t:case("two renamed databases fail closed instead of hiding notebooks", function()
        local controller = Controller.new{}
        controller._databasePath = function()
            return nil, "database_conflict",
                "/settings/justdraw-notebooks.sqlite3",
                "/settings/fingerink-notebooks.sqlite3"
        end
        local rows, err = controller:listNotebooks()
        t:eq(rows, nil, "library does not choose one history")
        t:eq(err, "database_conflict", "specific conflict reaches the UI")
        t:eq(controller.repository, nil, "no repository handle is retained")
    end)
