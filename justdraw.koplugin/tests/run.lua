...
t:case("a direct-ink cut keeps the stroke's style on every fragment", function()
    local s = Store.new()
    s:add(1, { n = 5, w = 2, t = 65, 0, 0, 25, 0, 50, 0, 75, 0, 100, 0 })
    t:eq(s:sweep(1, 50, -10, 50, 10, 5), true, "the cut succeeded")
    local list = s:get(1)
    t:eq(#list, 2, "two fragments from the cut")
    t:eq(list[1].t, 65, "head fragment keeps style")
    t:eq(list[2].t, 65, "tail fragment keeps style")
end)
