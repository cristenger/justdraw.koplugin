return function(ctx)
    ...
    t:case("a FileManager-shaped host touches no document API", function()
        ctx.reset()
        local plugin = support.newFileManagerPlugin(ctx.FingerInk, ctx.env)
    ...
    t:case("production activation includes FileManager", function()
        t:eq(ctx.FingerInk.is_doc_only, false, "notebooks are visible without a document")
    end)
    ...
    t:case("FileManager exposes only the document-free notebook entry", function()
        ...
        t:check(menu.fingerink_notebooks ~= nil, "Notebooks entry exists")
        t:eq(menu.fingerink_notebooks.text, "Notebooks", "entry is English")
        t:eq(menu.fingerink, nil, "reader-only drawing menu is absent")
    ...
    t:case("FileManager menu opens the real library without an input lease", function()
        ...
        local menu = {}
        plugin:addToMainMenu(menu)
        menu.fingerink_notebooks.callback()
    ...
    t:case("ReaderUI library leaves direct drawing untouched until a row opens", function()
        ctx.reset()
        local plugin = support.newPlugin(ctx.FingerInk, ctx.env)
        ctx.env.UIManager:flush()
