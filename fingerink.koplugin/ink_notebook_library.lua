...
function Library:_loadBatch(cursor, cursor_index, initial, consume_stale_generation)
    ...
    self.schedule(function()
        ...
        if not batch then
            ...
            logger.warn("FingerInk notebooks: library load failed:", err)
            self:_rebuild()
            return
        end
        self.batch = batch
...
function Library:_statusButton(width, height)
    ...
    elseif self.batch and self.batch.read_only_code and #self.batch.items == 0 then
        text = _("Read-only") .. "\n"
            .. _("This notebook library was created by a newer version of FingerInk. You can’t create or edit notebooks with this version.")
    ...
    return self:_button{
        ...
        enabled = self.load_error_code ~= nil,
        callback = self.load_error_code and function()
    ...
    if status then
        ...
        if banner_h > 0 then
            content[#content + 1] = self:_button{
                text = _("Read-only") .. "\n"
                    .. _("This notebook was created by a newer version of FingerInk. You can view it and change pages, but you can’t edit it."),
        ...
        buttons = {{
            ...
            { text = _("Create"), callback = function()
                ...
                if not notebook then
                    logger.warn("FingerInk notebooks: create failed:", err)
...
function Library:confirmDelete(item)
    ...
    box = ConfirmBox:new{
        ...
        ok_callback = function()
            ...
            if not ok then
                logger.warn("FingerInk notebooks: delete failed:", err)
